...
public class Admin {
    ...
    public HashMap<String, String> AddEmployee(@RequestBody Employee emp){
        List<Employee> emp1 = (List<Employee>) edao.UserLoginDetailsByCred(emp.getUsername(), emp.getPassword(), emp.getEmpCode());
        HashMap<String, String> hashMap = new HashMap<>();
        if(emp1.size()==0){
            LocalDateTime now = LocalDateTime.now();
            edao.save(emp);
            List<Employee> result = (List<Employee>) edao.UserLoginDetailsById(emp.getEmpCode());
            Leaves1 l1 = new Leaves1();
            l1.setEmpId(String.valueOf(result.get(0).getId()));
            l1.setYear(df.format(now));
    ...
    public List<Employee> SearchEmployee(@RequestBody Employee emp){
        return  (List<Employee>) edao.SearchEmployee(emp.getName());
    }
    ...
    public HashMap<String, String> EditEmployee(@RequestBody Employee emp){
        ...
        edao.EditEmployee(emp.getId(), emp.getDesignation(), emp.getEmail(), emp.getEmpCode(), emp.getName(), emp.getPassword(), emp.getPhone(), emp.getUsername());
        HashMap<String, String> hashMap = new HashMap<>();
        hashMap.put("status","success");
        return hashMap;
    }
